...
def loss(y_true, y_pred):
    return -K.log(dice_coef(y_true, y_pred)) + bce(y_true, y_pred)
...
def architecture(input_shape):
    ...
    conv2 = inception_block(pool1, 64, batch_mode=2)
    pool2 = ConvolutionPooling2D(conv2, 64, 3, 3, border_mode='same', subsample=(2, 2))
    ...
    model.compile(optimizer='adam', loss={'out': loss, 'out2x': loss, 'out4x': loss,
                  ...
                  loss_weights={'out': 1., 'out2x': 0.2, 'out4x': 0.05, 'aux_out': 0.5})
...
def main(num_epoch=10):
    ...
    for batch in datagen.flow(Xtrain, y=Ytrain, batch_size=BATCH_SIZE, shuffle=True, seed=SEED):
        ...
        model.fit(inputs, [targets, targets2x, targets4x, targets_aux], batch_size=bs, nb_epoch=1, shuffle=True)
        ypred = model.predict(Xval, batch_size=bs)[0]
        valid_score = np_dice_coef(Yval, ypred)
